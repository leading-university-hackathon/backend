...
def addDoctor_Serial(addDoctorSerial:schemas.addDoctorSerial, db:Session=Depends(database.get_db), current_user: models.User = Depends(oauth2.getCurrentUser)):
    ...
    doctor_serial = models.Doctor_Serial(type = addDoctor_Serial.type,user_id=current_user.id, doctor_id=doctor.user.id, date=datetime.now().date(), time=addDoctorSerial.time, appointmentDate=addDoctorSerial.appointmentDate)
